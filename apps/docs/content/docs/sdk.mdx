...
Use `@mitome/sdk` when your application is Promise-first. To scaffold a standalone Agent project, run `npm create mitome <directory>`. An Agent Definition contains Providers, one qualified Default Model id, and Extensions; `withSession` owns the Session's scoped lifetime.
