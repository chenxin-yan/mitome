...
Cleanup errors keep their existing semantics. On a failed exit they are logged so the primary tagged error survives; on a successful exit the remaining cleanups still run and the first error is then rethrown. A `defer` called after acquisition settled needs no guard: the callback lands on the same Session Scope and runs at close, so nothing leaks and no crust-style throw is warranted. A guard against reading `resource` after release is unreachable, because Hooks and handlers only run inside the Session and `isReleased` already blocks Turns.
