...
The selected TypeScript module default-exports `defineMitome({ agent, hosts })`. `defineMitome` and the public SDK `Host` interface compose one host-agnostic Agent Definition with its consumers; a Host currently has the single mode `interactive`, receives the Agent and staged prompt, and at most one may be declared. `tui(options)` returns such a Host. Installing `@mitome/tui` alone has no activation semantics.
