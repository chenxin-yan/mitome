...
Serialization remains a target invariant under [ADR-0057](0057-use-effect-native-functions-and-optional-host-composition.md). Revalidate the native controlled execution mechanism and first-party adapter overlap/order tests; do not add a second semaphore without evidence. The mechanism below describes current code, not proof of redesigned execution.
