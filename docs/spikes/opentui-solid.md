...
| NixOS Linux / Ghostty                                   | x86_64       | 1.3.14 | xterm-ghostty  | PASSED — manual checklist steps 1-5 all passed: flicker-free streaming, multiline paste retained, Tab focus, Down scroll, clean `q` exit and restore.                                                                                        |
| Real macOS terminal emulator(s)                         | deferred     | —      | —              | Deferred matrix extension; does not gate #73.                                                                                                                                                                                                |
| Compiled Bun executable Child Host                      | x86_64       | 1.3.14 | xterm-256color | PASSED — smoke compiled with `bun build --compile` so `process.execPath` is the compiled binary; the `BUN_BE_BUN` re-exec child loaded the native renderer, rendered, streamed, and exited 0 under `script(1)`. See resolution caveat below. |
